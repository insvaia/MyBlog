...
```js
Fucntion.prototype.MyBind = function (ctx, ...args){
    const fn = this
    return function (...restArgs){
        // 判断是否是new进行调用
        if(new.tagret){
            return new fn.apply(ctx, [...args, ...restArgs])
        }
...
function myNew(Constructor, ...args) {
  ...
  // 2. 使用 Reflect.construct 模拟 new 操作
  // 它会自动：
  //    - 创建一个新对象，并链接到 Constructor.prototype
  //    - 以 new 的方式调用 Constructor（对 class 合法）
  //    - 如果构造函数返回对象，则返回该对象；否则返回新创建的对象
  try {
    return Reflect.construct(Constructor, args);
  } catch (e) {
    // 如果 Constructor 是箭头函数或其他不可构造的值，会抛出 TypeError
    throw new TypeError(
      `${Constructor.name || "Constructor"} is not a constructor`,
    );
  }
}
